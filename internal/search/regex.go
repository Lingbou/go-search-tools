...
// NewRegexSearcher 创建一个新的正则表达式搜索器
func NewRegexSearcher(cfg *config.SearchConfig, pattern string) *RegexSearcher {
	// 创建过滤器
	...
	// 创建正则表达式匹配器
	regexMatcher := matcher.NewRegexMatcher(pattern, cfg.IgnoreCase)
...
// Search 执行正则表达式搜索
func (s *RegexSearcher) Search() error {
	// 检查路径是否存在
